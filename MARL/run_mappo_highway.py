...
def parse_args():
    ...
    Description for this experiment:
        ...
        + seed = 0
    """
    default_base_dir = r"./results/"
    default_config_dir = r'configs/configs_ppo.ini'
...
def train(args):
    ...
    # env.config['right_lane_reward'] = config.getint('ENV_CONFIG', 'RIGHT_LANE_REWARD')
    ...
    # env_eval.config['right_lane_reward'] = config.getint('ENV_CONFIG', 'RIGHT_LANE_REWARD')
...
def evaluate(args):
    if os.path.exists(args.model_dir):
        model_dir = args.model_dir + r'/models/'
    else:
        raise Exception("Sorry, no pretrained models")
    config_dir = args.model_dir + r'/configs/configs_ppo.ini'
    config = configparser.ConfigParser()
    config.read(config_dir)

    video_dir = args.model_dir + r'/eval_videos'
